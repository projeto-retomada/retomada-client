<template>
  <div>
    ...
    <CModal title="Novo Usuário" size="xl" :show.sync="largeModal">
      <form class="row">
        ...
        <div class="col-lg-6 col-sm-12 mt-1">
          ...
          <select name="role" v-model="classAdd" class="form-control">
            <option v-for="(item) in usergroups" :key="item.id_usergroup" :value="item.id_usergroup">
              {{ item.name }}
            </option>
          </select>
        </div>
    ...
    <CModal title="Editar Usuário" size="xl" :show.sync="editModal">
      <form class="row">
        <div class="col-lg-6 col-sm-12 mt-1">
          <label for="username">Username *</label>
          <input
        ...
        <div class="col-lg-6 col-sm-12 mt-1">
          <label for="role">Role</label>
          <select name="roleEdit" v-model="roleEdit" class="form-control">
            <option value="ADMIN">Administrador</option>
            <option value="STUDENT">Aluno</option>
            <option value="TEACHER">Professor</option>
          </select>
        ...
        <div class="col-lg-6 col-sm-12 mt-1">
          <label for="role">Turma</label>
          <select name="role" v-model="classAddEdit" class="form-control">
            <option v-for="(item) in usergroups" :key="item.id_usergroup" :value="item.id_usergroup">
              {{ item.name }}
            </option>
          </select>
        ...
        <div class="col-lg-6 col-sm-12 mt-1">
          ...
          <div class="form-check form-check-inline">
            <input
              class="form-check-input"
              type="radio"
              name="group_risk_edit"
              value="S"
              v-model="group_riskEdit"
            />
            <label class="form-check-label">Sim</label>
          </div>
          <div class="form-check form-check-inline">
        ...
        <div class="col-lg-6 col-sm-12 mt-1">
          ...
          <input
            type="passwordEdit"
            name="passwordEditLast"
            v-model="passwordEditConfim"
            class="form-control"
          />
        </div>
      </form>
      <template #footer>
        <CButton @click="editModal = false" color="danger">Cancelar</CButton>
        <CButton
          @click="
            editModal = false;
            putUsuario();
